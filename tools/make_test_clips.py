...
BASES = {
    ...
    # Пролом намеренно короче своего куска (3 с против 6.2 с) и с бегущим
    # счётчиком: если после третьей секунды картинка замерла — удержание
    # последнего кадра работает, если счётчик пошёл заново — клип закольцевался,
    # и дверь вылетела второй раз.
    "breach": "testsrc=size=1920x1080:rate=30",
}
