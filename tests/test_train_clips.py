...
def test_a_clip_has_exactly_one_panel_per_beat(clips):
    for clip in clips:
        assert len(clip.panels) <= MAX_REFS
        # число долей клипа восстанавливается из его же промпта: он объявляет,
        # сколько картинок ждёт, и разойтись с панелями не должен
        assert "images 1-%d" % len(clip.panels) in clip.prompt or \
               "images 1 and 2" in clip.prompt
...
def test_more_panels_than_the_model_takes_is_loud(strikes, raw, tmp_path):
