...
def sum_stems(stem_paths: list[Path], tl: Timeline, out_path: Path) -> None:
    """Честная сумма стемов без лимитера — и потому в 32-битный float.

    amix здесь с normalize=0, то есть стемы складываются как есть, без деления
    на их число. Сумма законно может выйти за полную шкалу: у неё нет причин
    туда не выходить, потолок появляется только на мастере.

    В целочисленном pcm_s24le такой выход срезался бы прямо при записи, а
    вернуть срезанное нечем: normalize идёт с linear=true, один линейный гейн,
    он делает тише, а не восстанавливает форму. Замер на сборке lo_v41 это и
    показал — сумма упиралась в потолок на 0.36 dB, и на 55.478 с, где
    последние слова номера накладывались на ледяной удар, девять отсчётов
    срезало насовсем.

    У float потолка нет. Приводит сумму к цели loudnorm своим ограничителем
    истинного пика, то есть там, где это и должно происходить. Премастер
    никуда не сдаётся, он промежуточный, и его размер значения не имеет.
    Стемы и мастер остаются 24-битными.
    """
    cmd = ["ffmpeg", "-y", "-hide_banner", "-nostats", "-loglevel", "error"]
    ...
    cmd += [
        ...
        "-ar", str(tl.sample_rate), "-ac", "2", "-c:a", "pcm_f32le",
        str(out_path),
    ]
