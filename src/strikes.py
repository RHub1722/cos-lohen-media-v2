...
class Beat:
    ...
    cue: str = ""
    # Ставить ли нарастающий шум перед этим контактом. По умолчанию да — на
    # каждый. Исключение живёт здесь, рядом с долей, о которой оно говорит: у
    # встречного удара вспышки 4 замаха нет намеренно, и объявлять подготовку,
    # которой не существует, значит врать о движении. Причина пишется в поле
    # `riser_why` того же объекта. Разбор ризов — в src/counting.py.
    riser: bool = True
