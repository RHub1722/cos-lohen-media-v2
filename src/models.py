...
class Event:
    ...
    def from_dict(raw: dict) -> "Event":
        ...
        # Уводить музыку можно под удар и под реплику. Под музыкой бессмысленно,
        # под атмосферой тоже: она и так на −22…−26 dB и ничего не перебивает.
        #
        # Голос сюда добавлен после замера, а не сразу. Запас реплик боя над
        # музыкой оказался 1.7 dB у «Finally» и 2.4 у «...Really?», при 21–38 dB
        # у тех же реплик в допросе. То есть под подложку боя ушёл весь речевой
        # слой. Удар зал может достроить по картинке, реплику нет.
        if duck_db and stem not in ("sfx", "voices"):
            raise ScenarioError(
                f"{event_id}: duck_db на стеме {stem!r}. Уводить музыку под "
                "музыкой бессмысленно, а атмосфера и так тише всего."
            )
