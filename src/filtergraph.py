...
# Форма провала музыки, в секундах. Атака начинается раньше самого события
# намеренно: замах стоит за 0.30–0.45 с до удара, и музыка должна уйти вниз уже
# под замахом, иначе слышно, как она дёргается посередине движения.
DUCK_ATTACK = 0.30
DUCK_HOLD = 0.30
DUCK_RELEASE = 0.50

# Под репликой полка держится дольше: удар длится сотые доли, а фраза — от 0.8 до
# 1.6 с. С полкой в 0.30 музыка возвращалась бы на середине слова, и это слышно
# хуже, чем если бы её не уводили вовсе.
DUCK_HOLD_VOICE = 1.20
...
def duck_expression(tl: Timeline) -> str:
    ...
    Форма трапеция: линейный вход за DUCK_ATTACK до события, полка, линейный
    выход за DUCK_RELEASE. Резкая ступенька щёлкает, а плавная кривая здесь не
    нужна: под ударом всё равно грохот.

    Полка зависит от того, что уводит музыку: удар длится сотые доли, реплика
    больше секунды. Один и тот же размер полки для обоих означал бы либо музыку,
    возвращающуюся посреди слова, либо провал вчетверо длиннее удара.
    ...
    for ev in sorted(ducks, key=lambda e: e.t):
        hold = DUCK_HOLD_VOICE if ev.stem == "voices" else DUCK_HOLD
        start = ev.t - DUCK_ATTACK
        end = ev.t + hold + DUCK_RELEASE
