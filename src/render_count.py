...
# Уровень риза здесь -11 dB, а не -9 как на дорожках со счётом, и это тоже
# замер, а не вкус. Провал отпускает РОВНО в контакт, поэтому вершина риза и
# сам удар складываются в одной точке: на +3 dB это давало 4 отсчёта у потолка
# на 44.975 с. Подобрано по замеру — на -2 выходит -2.04 dBTP без единого
# отсчёта за пределом.
#
# Слышимость мерить общей громкостью нельзя — так и было сделано сначала, и
# замер показал, что в окне риза дорожка на 1.6-3.6 dB ТИШЕ голого номера:
# провал убирает больше, чем добавляет риз. Верная мера — полоса самого риза.
# В 1-6 кГц дорожка выше номера на +1.2...+4.9 dB, а ниже 200 Гц на -4.5. Низ
# отступает, верх набухает; слабее всего на вспышке 3, там всего +1.2.
#
...
TRACKS = (
    ...
    {"key": "riser", "name": "только риз, без счёта", "count": False,
     "pan": PAN_BOTH, "duck": "risers", "cue_db": -2.0,
...
def riser_duck(rows: list[dict]) -> str:
    ...
    for row in rows:
        a = row["start"]
        # Отпускание кончается РОВНО в контакт, а не вокруг него. Симметричное
        # окно оставляло на самом ударе половину глубины — около 4 dB, — и это
        # ровно тот звук, под который надо попасть. Заметно стало, когда риз
        # второго попадания вспышки 3 начался в момент первого.
        b = row["peak"]
