...
## Ризы под теми же якорями

Риз — нарастающий шум, вершина которого приходится ровно в контакт. Он даёт не
точку, как слово, а разгон: слышишь, как оно набирает, и попадаешь в вершину.
Ризы стоят на КАЖДОМ контакте, восемь на номер, против шести слов.

```bash
python src/render_count.py --cues --only riser
```

Кладёт в `output/cues/` шесть файлов `lohen_riser_<якорь>[_sync].m4a` — те же
три якоря и те же сверочные копии, что у слов. Ловить и мерить ими одинаково.
