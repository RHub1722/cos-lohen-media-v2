...
| `tools/atlas_gen.py` | создаётся. Загрузить, отправить, дождаться, скачать, обеззвучить, записать в журнал. **После пробы дополнен:** каждая попытка ложится отдельным файлом в `assets/video/attempts/`, в слот уходит копия; `--use ЯКОРЬ=N` переключает слот без генерации; `--refetch <id> --as ЯКОРЬ` достаёт уже оплаченный результат по идентификатору |
| `docs/atlas-ledger.csv` | создаётся первым запуском. Колонка `prediction_id` пишется сразу после отправки задания, до скачивания: упавшая загрузка тоже обязана оставлять след, по которому файл возвращается бесплатно |
